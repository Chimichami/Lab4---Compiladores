...
Stm* Parser::parseStm() {
    ...
    else if (check(Token::SWITCH)){
        a = parseSwitch();
    }
    else{
        cout<< "Error: sentencia no reconocida." << endl;
        throw runtime_error(current->text);
    }
    return a;
}
...
Stm* Parser::parseSwitch(){
    Exp* e;
    Exp* c;
    Body* s;
    list<CaseStmt*> l;
    match(Token::SWITCH); //switch
    match(Token::LPAREN);// (
    e = parseCE(); //switch(y)
    match(Token::RPAREN);
    while(match(Token::CASE)){
        c = parseCE(); //case c
        match(Token::TWOPOINTS);
        s = parseBody();
        l.push_back(new CaseStmt(c,s));
    }   
    match(Token::ENDSWITCH);
    return new SwitchStm(e,l);
}
