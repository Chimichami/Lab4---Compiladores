...
int CaseStmt::accept(Visitor* visitor){
    return visitor->visit(this);
}
...
int GenCodeVisitor::visit(IfStm* stm) {
    if (stm->condition->isnumber and stm->condition->valor!=0)
    {
...
int GenCodeVisitor::visit(SwitchStm* stm) {
    int label = labelcont++;
    stm->switch_exp->accept(this); //%rax
    out << " movq %rax, %rcx"<<endl; // guarda el valor del switch en %rcx
    for (auto c: stm->case_list){
        c->accept(this);
    }
    out << " jmp endswitch_" << label << endl;
    // out << "case_" << label << ":"<< endl;
    out << "endswitch_" << label << ":"<< endl;
    return 0;
}
int GenCodeVisitor::visit(CaseStmt* exp) {

    exp->case_val->accept(this); // deja en %rax el valor del case
    out << "cmpq %rax, %rcx"<<endl; // compara con el valor del switch
    int label = labelcont++;
    out << "jne endcase_" << label << endl;
    exp->then->accept(this);
    out << "endcase_" << label << ":"<< endl;
    return 0;

    // if (exp->condition->isnumber and exp->condition->valor!=0){
    //     exp->then->accept(this);
    // }
    // else{
...
int TypeCheckerVisitor::visit(CaseStmt* exp) {
    return 0;
}
...
int TypeCheckerVisitor::visit(SwitchStm* exp) {
    return 0;
}   